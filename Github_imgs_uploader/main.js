...
['token','user','repo','branch'].forEach(k=>{ if(CONFIG[k]) $(`#${k}`).value=CONFIG[k]; });
...
async function upload({file,dom,status,urlBox,retryBtn}){
  const {token,user,repo,branch} = CONFIG;
  if(!token||!user||!repo||!branch){
    status.textContent='❌ 请先填写配置'; status.classList.add('error'); return;
  }
  status.textContent='上传中…'; status.classList.remove('success','error');
  ...
  try{
    ...
    const body = JSON.stringify({
      message: `upload ${fileName} (by gh-uploader)`,
      content: content,
      branch
    });
